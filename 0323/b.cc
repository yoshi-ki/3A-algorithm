...
int main() {
  ...
  rep(i,t){
    ...
    rep(j,n){
      int K;
      cin >> K;
      if(K==0) {if(flag2) {a1 = j+1;flag2=0;} continue;}
      int flag = 1;
      rep(k,K){
        int male;
        cin >> male;
        if(flag ==0) continue;
        if(mpm[male] == 0) {mpm[male] = 1; flag = 0;}
        else continue;
      }
      if(flag) {if(flag2) {a1 = j+1;flag2=0;}}//答えに使う女
    }

    if(a1==0) cout << "OPTIMAL" << endl;
    else{
      int a2 = 0;
      rep(j,n){if(mpm[j+1] == 0) {a2 = j+1; break;}}
      cout << "IMPROVE" << endl;
      cout << a1 << " " << a2 << endl;
    }
