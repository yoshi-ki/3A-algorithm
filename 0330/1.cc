...
int main() {
  //これは全ての二分探索はできないから、うまい具合に単調性を持して二分探索できる形にしてあげればよかったなぁというお気持ちになった。
  ll n,k;
  cin >> n >> k;
  vector<ll> a(n);
  vector<ll> b(n);
  ...
  while(l < r - 1){
    auto binarysearch = [&](ll X){
      ...
      ll res = 0;
      rep(i,n){
        res = res + (upper_bound(all(b),X/a[i]) - b.begin());
      }
      return res;
    };
    ll num = binarysearch((r+l)/2);
    cout << "searching " << (r+l)/2 << " and result is " << num << endl;
    if(num >= k) r = (r+l)/2; //今は最小のxを求めたいので、条件を満たしている場合もrを移動させてあげる必要がある
    //逆に条件を満たす最大のxを求める場合であれば、lの方に等号成立を持ってきて上げて、lを動かしていく必要がある
    else l = (r+l)/2;
  }
  cout << r << endl;
  return 0;
