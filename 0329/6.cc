...
int main() {
  ...
  SegTree<pair<double,double>> tree(size,pair<double,double>(1,0),[](pair<double,double> a, pair<double,double> b){
    return pair<double,double> (a.first*b.first,a.second * b.first + b.second);
  });

  double Min = 1.0;
  double Max = 1.0;
  rep(i,m){
    int id = lower_bound(all(pls),p[i]) - pls.begin();
    tree.update(id,pair<double,double>(a[i],b[i]));
    pair<double,double> res = tree.query(0,size);
    Min = min(Min,res.first + res.second);
    Max = max(Max,res.first + res.second);
  }

  cout << fixed << setprecision(14) << Min  << endl << Max << endl;
