...
int main() {
  ...
  rep(test,t){
    ...
    vector<ll> a(n);
    ll amax = 0;
    rep(i,n) {cin >> a[i]; amax = max(amax,a[i]);}

    //mpに使ったかどうかの情報を保存しておく, 初期化はamaxを用いて行なっておく
    map<int,int> mp;
    int templog = 1;
    int amaxlog = 0;
    while(templog <= amax) {templog *= k; amaxlog++;}
    rep(i,amaxlog+3) mp[i] = 0;

    //judgeする関数, a[j]について作業を行う
    auto judge = [&](int j) {
      int flag = 1;
      while(flag==1){
        ll temp = 1;
        ...
        while(temp*k <= a[j]) {temp *= k; count++;}
        if(count == 0) {
          if(mp[count] == 1) flag = 2;
          else{
            mp[count] = 1; flag = 0;
          }
        }else{
          if(mp[count] == 1) {flag = 2;}
          else{mp[count] = 1; a[j] = a[j] - temp; continue;}
        }
      }
      if(flag == 0) return true; else return false;
    };

    //これ以降で実際に作業を行なって結果をだす
    int flag2 = 1;
    rep(i,n){bool hoge = judge(i); if(!hoge) {flag2 = 0; break;}}
    if(flag2) cout << "YES" << endl; else cout << "NO" << endl;
  }
  return 0;
}
